...
- Dynamic typing language
  ...
  - **변수에 자료형을 정해주지 않는다.**
  - 간결하고 편리하다.
  - 코드의 어려운 표현이 적어서 **코드의 가독성이 좋다**.
  - **작은 사이즈의 프로그램을 만들기 적합**하다.
  - 컴퓨터적 구조가 생략되어 **실행 속도가 느리다**.
  - 실행속도가 민감하지 않은 프로젝트에 적합.



- Static typing language
  - C, Cpp, Kotlin, Java 등이 있다.
  - **변수에 자료형을 미리 정해준다.**
  - 명확하고 정교하다.
  - 컴퓨터적 구조가 딱 눈에 들어온다.
  - 제약이나 규칙이 많다. 실수를 방지하고 더 체계적으로 코드를 작성하기 위함이다. 
    따라서, **협업해야하는 프로젝트에 좋다**.
  - 컴퓨터적 구조를 많이 적어줘서 **실행 속도가 빠르다**.



### 1. Dynamic typing과 Static typing의 장단점

![image-20200228014306980](./assets/image-20200228014306980.png)
...
Dynamic typing 언어는 간편함과 편리함을 추구하며, 컴퓨터적 구조를 생략하기 때문에 코드의 내용(로직)이 눈에 잘 들어오고 사람이 코드를 이해하기 편해서 학습하기가 쉽습니다.   다만 컴퓨터적 구조를 생략하기 때문에 실행 속도에 민감한 곳이나 환경이 열악한 환경에서는 활약하기 힘듭니다.
