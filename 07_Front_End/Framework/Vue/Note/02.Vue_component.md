...
### # [전역 등록](https://kr.vuejs.org/v2/guide/components.html#전역-등록)
...
> Vue는 사용자 지정 태그 이름에 대해 [W3C 규칙](https://www.w3.org/TR/custom-elements/#concepts)을 적용하지 않습니다 (모두 소문자이어야 하고 하이픈을 포함해야합니다). 그러나 이 규칙을 따르는 것이 좋습니다.
...
![image-20200527165030166](assets/component_1.png)

   

### # [지역 등록](https://kr.vuejs.org/v2/guide/components.html#지역-등록)
...
### # [`data` 는 반드시 함수여야합니다.](https://kr.vuejs.org/v2/guide/components.html#data-는-반드시-함수여야합니다)
...
![image-20200527165607428](assets/component_2.png)
...
![image-20200527165430043](assets/component_3.png)

  

### # [컴포넌트 작성](https://kr.vuejs.org/v2/guide/components.html#컴포넌트-작성)
...
### # [Props로 데이터 전달하기](https://kr.vuejs.org/v2/guide/components.html#Props로-데이터-전달하기)
...
![image-20200527165736332](assets/component_4.png)

{% endraw %}

   

### # [camelCase vs. kebab-case](https://kr.vuejs.org/v2/guide/components.html#camelCase-vs-kebab-case)
...
### # [동적 Props](https://kr.vuejs.org/v2/guide/components.html#동적-Props)
...
![image-20200527165903948](assets/component_5.png)
...
### # [리터럴 vs. 동적](https://kr.vuejs.org/v2/guide/components.html#리터럴-vs-동적)
...
### # [Prop 검증](https://kr.vuejs.org/v2/guide/components.html#Prop-검증)
...
### # [존재하는 속성 교체/병합](https://kr.vuejs.org/v2/guide/components.html#존재하는-속성-교체-병합)
...
### # [`v-on`을 이용한 사용자 지정 이벤트](https://kr.vuejs.org/v2/guide/components.html#v-on을-이용한-사용자-지정-이벤트)

모든 Vue 인스턴스는 다음과 같은 [이벤트 인터페이스](https://kr.vuejs.org/v2/api/#Instance-Methods-Events)를 구현합니다.
...
>  Vue의 이벤트 시스템은 브라우저의 [EventTarget API](https://developer.mozilla.org/en-US/docs/Web/API/EventTarget)와 별개입니다. 비슷하게 작동하지만 `$on` 과 `$emit` 는 `addEventListener` 와 `dispatchEvent`의 별칭이 **아닙니다**.
...
![image-20200527170205877](assets/component_6.png)
...
### # [`.sync` 수식어](https://kr.vuejs.org/v2/guide/components.html#sync-수식어)
...
### # [사용자 정의 이벤트를 사용하여 폼 입력 컴포넌트 만들기](https://kr.vuejs.org/v2/guide/components.html#사용자-정의-이벤트를-사용하여-폼-입력-컴포넌트-만들기)
...
![image-20200527170434368](assets/component_7.png)

위의 구현은 꽤 단순합니다. 예를 들어, 사용자는 때때로 여러 마침표와 글자를 입력 할 수 있습니다. 그렇기 때문에 보다 강력한 통화 필터를 사용할 수 있습니다.

  

### # [컴포넌트의 `v-model` 사용자 정의](https://kr.vuejs.org/v2/guide/components.html#컴포넌트의-v-model-사용자-정의)
...
### # [범위 컴파일](https://kr.vuejs.org/v2/guide/components.html#범위-컴파일)
...
### # [단일 슬롯](https://kr.vuejs.org/v2/guide/components.html#단일-슬롯)
...
### # [이름을 가지는 슬롯](https://kr.vuejs.org/v2/guide/components.html#이름을-가지는-슬롯)
...
### # [범위를 가지는 슬롯](https://kr.vuejs.org/v2/guide/components.html#범위를-가지는-슬롯)
...
### # [`keep-alive`](https://kr.vuejs.org/v2/guide/components.html#keep-alive)
...
[API 참조](https://kr.vuejs.org/v2/api/#keep-alive)에서 `<keep-alive>` 에 대한 자세한 내용을 확인하십시오.
...
### # [재사용 가능한 컴포넌트 제작하기](https://kr.vuejs.org/v2/guide/components.html#재사용-가능한-컴포넌트-제작하기)
...
### # [자식 컴포넌트 참조](https://kr.vuejs.org/v2/guide/components.html#자식-컴포넌트-참조)
...
### # [비동기 컴포넌트](https://kr.vuejs.org/v2/guide/components.html#비동기-컴포넌트)
...
[지역 등록](https://kr.vuejs.org/v2/guide/components.html#Local-Registration)을 사용하는 경우, `Promise`를 반환하는 함수를 제공할 수 있습니다.
...
### # [고급 비동기 컴포넌트](https://kr.vuejs.org/v2/guide/components.html#고급-비동기-컴포넌트)
...
### # [컴포넌트 이름 규약](https://kr.vuejs.org/v2/guide/components.html#컴포넌트-이름-규약)
...
이것은 PascalCase가 가장 보편적 인 *선언적 컨벤션*임을 의미하며 케밥 케이스는 가장 보편적으로 사용하는 *컨벤션*입니다.
...
### # [재귀 컴포넌트](https://kr.vuejs.org/v2/guide/components.html#재귀-컴포넌트)
...
### # [컴포넌트 사이의 순환 참조](https://kr.vuejs.org/v2/guide/components.html#컴포넌트-사이의-순환-참조)
...
### # [X-Templates](https://kr.vuejs.org/v2/guide/components.html#X-Templates)
...
### # [`v-once`를 이용한 비용이 적게드는 정적 컴포넌트](https://kr.vuejs.org/v2/guide/components.html#v-once를-이용한-비용이-적게드는-정적-컴포넌트)
